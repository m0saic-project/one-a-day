...
5. Render and snapshot the variant:
   ...
   It builds, renders landscape/portrait/square, cuts stills, copies the source
   folder, and writes `report.json`. Exit 3 means an ERROR MOSAIC rendered —
   that variant is broken however green the build was. Look at the stills if
   you can view images; read `report.json` regardless.
