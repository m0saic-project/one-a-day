...
async function render(props: BenchDeltaProps, ctx: MosaicEngineContext): Promise<MosaicDocument> {
  ...
  const text = (rect: BenchRect, importance: number, fit: BenchFit, color: MosaicColor, label: string, o: { hAlign?: "left" | "right"; bold?: boolean; vAlign?: "top" | "middle" } = {}) =>
  ...
  L.rows.forEach((r, i) => {
    ...
    text(r.verdictRect, 2, r.verdictFit, r.verdict.kind === "unknown" ? theme.dim : barColor(r.verdict.kind), `verdict-${i}`, { hAlign: "right", bold: true });
  });
