...
describe(ID, () => {
  ...
  it("prints MM:SS from one drawtext expression and gates the three phase copies to windows that meet exactly", async () => {
    ...
    // the frame tint follows the same two windows, and goes away with the knob
    expect(by("tint-warn")).toEqual([{ label: "tint-warn", enable: "gte(t,240)*lt(t,270)", window: { startSec: 240, endSec: 270 } }]);
    expect(by("tint-final")).toEqual([{ label: "tint-final", enable: "gte(t,270)", window: { startSec: 270 } }]);
    const quiet = gates(await render({ phaseTint: false }));
    expect(quiet.filter((x) => x.label.startsWith("tint-"))).toHaveLength(0);
    ...
    // ...and translucent, so the frame's tint reads through a spent slice
    for (const s of (bar?.sources ?? []) as Array<{ editor?: { label?: string }; visual?: { opacity?: number } }>) {
      if (s.editor?.label === "seg-spent") expect(s.visual?.opacity).toBeLessThan(0.5);
    }
  });
