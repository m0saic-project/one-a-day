...
export function layoutAudiogram(opts: AudiogramOpts, W: number, H: number): AudiogramLayout {
  ...
  const refls: AgRect[] = opts.wave.map((amp, k) => {
 ...
 */
function registerMedia(assets: NonNullable<MosaicDocument["assets"]>, ref: string, fixedId: string, mediaType: "audio" | "image") {
  const id = asAssetId(ASSET_ID_RE.test(ref) ? ref : fixedId);
...
async function render(props: EpisodeAudiogramProps, ctx: MosaicEngineContext): Promise<MosaicDocument> {
  ...
  L.refls.forEach((r) => {
    barPieces.push({ rect: { ...r, importance: 1 }, source: tag(makeColorTile(barRefl) as MosaicSource, "bar-refl") });
  });
  L.bars.forEach((r, k) => {
    const at = L.litAt[k];
