 ...
 * (1-2 lines)] · margin · footer (site left, author right), with a vertical
 * accent rule on the LEFT of the text block (an editorial pull-rule). The
 * text block centres vertically between the top margin and the footer, so
 * square and portrait canvases do not spread the rows apart. Every drawn
 * prop is bound to its rect (Make double-click edits it in place); the rule
...
export function layoutOgCard(
  ...
  // The accent is a vertical rule on the left of the text block; the block
  // moves right by the rule and a gap, the footer stays on the margin.
  const ruleW = Math.max(4, Math.round(0.012 * S));
  const ruleGap = Math.max(6, Math.round(0.035 * S));
  const textX = margin + ruleW + ruleGap;
  const interiorW = Math.max(16, W - textX - margin);
  ...
  // The rule spans the stack, from the kicker's top to the last row's bottom.
  const bar: Rect = { x: margin, y: stackTop, w: ruleW, h: stackH };
...
export const OgCardV1 = defineMosaicTemplate<OgCardProps>({
  ...
  async render(props: OgCardProps, ctx: MosaicEngineContext): Promise<MosaicDocument> {
    ...
    // The rule shows `accent` - it is the swatch handle.
