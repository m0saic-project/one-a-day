...
exports.BenchDeltaV1 = exports.SAMPLE_ROWS = exports.MAX_ROWS = void 0;
exports.toNum = toNum;
exports.verdictOf = verdictOf;
exports.fmtNum = fmtNum;
exports.fmtRatio = fmtRatio;
exports.verdictText = verdictText;
exports.mix = mix;
exports.layoutBenchDelta = layoutBenchDelta;
exports.benchLayoutContract = benchLayoutContract;
exports.contrastRatio = contrastRatio;
exports.onColor = onColor;
...
const propsSchema = (0, template_utils_1.definePropsSchema)({
    rows: {
        type: "list",
        required: true,
        description: "The benchmark cases, in the order they should read. Each row is the pair of runs for one benchmark: name, unit, the baseline number and the candidate number, plus the run-to-run variance of each (+/-). A row with no variance on either side gets no verdict - that is deliberate. Up to 8 rows are drawn.",
 ...
 */
function verdictOf(row, smallerIsBetter) {
    const base = pos(row.base);
    const v = toNum(row.value);
    const value = v !== null && v >= 0 ? v : null;
    if (base === null || value === null)
        return { kind: "unknown", ratio: 1, missing: "both", reason: "a measurement is missing" };
    ...
    const v1 = value + vR;
    if (b0 <= v1 && v0 <= b1)
        return { kind: "noise", ratio, missing: null, reason: "the intervals overlap" };
...
function verdictText(v) {
    if (v.kind === "unknown")
        return v.missing === "both" ? "no spread given" : "one side unmeasured";
    if (v.kind === "noise")
        return "within noise";
...
function fitOne(text, maxW, maxPx, minPx, bold = false) {
    ...
    return { text: out, px, width: (0, text_1.widthOf)(out, px, bold), lines: 1 };
}
/**
 ...
 */
function sharedLinePx(texts, maxW, maxPx, minPx, bold = false) {
    let px = Math.max(minPx, Math.round(maxPx));
    while (px > minPx && texts.some((t) => (0, text_1.widthOf)(t, px, bold) > maxW))
        px = Math.max(minPx, Math.round(px * 0.92));
    return px;
}
/** {@link sharedLinePx} for blocks that may wrap: every one must fit its box in `maxLines`. */
function sharedBlockPx(texts, maxW, maxH, maxPx, minPx, maxLines, bold = false) {
    const fitsAll = (px) => texts.every((t) => {
        const lines = (0, text_1.wrapFit)(t, px, maxW);
        return (lines.length <= maxLines &&
            Math.max(...lines.map((l) => (0, text_1.widthOf)(l, px, bold))) <= maxW &&
            Math.ceil(px * 1.25 * lines.length) <= maxH);
    });
    let px = Math.max(minPx, Math.round(maxPx));
    while (px > minPx && !fitsAll(px))
        px = Math.max(minPx, Math.round(px * 0.92));
    return px;
}
/**
 ...
 */
function fitSegments(segments, maxW, maxPx, minPx) {
    var _a;
    for (let n = segments.length; n >= 1; n--) {
        ...
        if ((0, text_1.widthOf)(text, px) <= maxW)
            return { fit: { text, px, width: (0, text_1.widthOf)(text, px), lines: 1 }, dropped: segments.length - n };
    }
    // Even the first clause alone does not fit: shrink it and, only here, cut it.
    const one = fitOne((_a = segments[0]) !== null && _a !== void 0 ? _a : "", maxW, maxPx, minPx);
...
function fitLines(lines, maxW, maxPx, minPx, bold = false) {
    const wid = (p, ls) => Math.max(...ls.map((l) => (0, text_1.widthOf)(l, p, bold)));
    let px = Math.max(minPx, Math.round(maxPx));
    while (px > minPx && wid(px, lines) > maxW)
        px = Math.max(minPx, Math.round(px * 0.92));
    const out = wid(px, lines) > maxW ? lines.map((l) => (0, text_1.ellipsize)(l, px, maxW, bold)) : lines;
 ...
 */
function fitBlock(text, maxW, maxH, maxPx, minPx, maxLines, bold = false) {
    const attempt = (px) => {
        const lines = (0, text_1.wrapFit)(text, px, maxW);
        if (lines.length > maxLines)
            return null;
        const block = lines.join("\n");
        const w = Math.max(...lines.map((l) => (0, text_1.widthOf)(l, px, bold)));
    ...
    const all = (0, text_1.wrapFit)(text, minPx, maxW);
    ...
    if (all.length > keep) {
        ...
        lines[keep - 1] = (0, text_1.ellipsize)(all.slice(keep - 1).join(" "), minPx, maxW, bold);
    }
    const block = lines.join("\n");
    return { text: block, px: minPx, width: Math.max(...lines.map((l) => (0, text_1.widthOf)(l, minPx, bold))), lines: lines.length };
}
/**
 ...
 */
function layoutBenchDelta(rows, opts, W, H) {
    ...
    const title = fitOne(opts.title, (0, text_1.budget)(titleW), Math.round(0.058 * S), minPx, true);
    const subPx = Math.max(minPx, Math.round(0.027 * S));
    const subtitle = opts.subtitle.length > 0 ? fitOne(opts.subtitle, (0, text_1.budget)(titleW), subPx, minPx) : null;
    ...
    const titleRect = { x: margin, y: margin, w: titleW, h: titleH };
    const subtitleRect = { x: margin, y: margin + titleH, w: titleW, h: Math.max(1, subH) };
    const summaryRect = { x: margin + contentW - sumW, y: margin, w: sumW, h: headerH };
    const dividerH = Math.max(2, Math.round(0.0035 * S));
    const divider = { x: margin, y: margin + headerH + Math.round(0.45 * margin), w: contentW, h: dividerH };
    ...
    const footerRect = { x: margin, y: H - margin - footH, w: contentW, h: footH };
    ...
    let shown = Math.min(rows.length, exports.MAX_ROWS);
    while (shown > 1 && Math.floor(bandH / shown) < minPitch)
        shown -= 1;
    ...
    const nameCapFor = (h) => Math.max(minPx, Math.min(Math.round(0.03 * S), Math.round(h * 0.42)));
    const nameBoxHFor = (h) => Math.round(h * 0.56);
    const labelFrac = (() => {
        var _a;
        ...
        const tried = [0.28, 0.32, 0.36].map((f) => {
            const w = (0, text_1.budget)(Math.round(contentW * f));
        ...
        if (whole.length > 0)
            return ((_a = whole.find((t) => t.px >= comfortable)) !== null && _a !== void 0 ? _a : whole[0]).f;
    ...
    const nameBoxW = (0, text_1.budget)(labelW);
    const nameBoxH = nameBoxHFor(rowH);
    const numbersOf = (r) => `${fmtNum(r.base)} -> ${fmtNum(r.value)}`;
    const valuesOf = (r) => {
    ...
    const valWidth = (0, text_1.budget)(labelW);
    const withUnit = sharedLinePx(drawn.map(valuesOf), valWidth, valCap, minPx);
    const unitFits = drawn.every((r) => (0, text_1.widthOf)(valuesOf(r), withUnit) <= valWidth);
    ...
    const verdPx = sharedLinePx(verdicts.map(verdictText), (0, text_1.budget)(verdW), verdCap, minPx, true);
    const out = drawn.map((row, i) => {
        var _a;
        const verdict = verdicts[i];
        const y = blockTop + i * pitch;
        const b = (_a = pos(row.base)) !== null && _a !== void 0 ? _a : 1;
        const relPx = (v) => Math.max(0, Math.min(trackW, Math.round((Math.max(0, v) / b) * ref)));
        const ivOf = (v, r, slotY, h) => {
        ...
        const baseBar = { x: trackX, y: baseY, w: Math.max(minBar, relPx(row.base)), h: barH };
        const candBar = { x: trackX, y: candY, w: Math.max(minBar, relPx(row.value)), h: candH };
        ...
        const values = fitOne(valueText(row), (0, text_1.budget)(labelW), valPx, minPx);
        ...
        const nameRect = { x: margin, y: blockY, w: labelW, h: nameH };
        const valuesRect = { x: margin, y: blockY + nameH, w: labelW, h: valH };
        const verdictFit = fitOne(verdictText(verdict), (0, text_1.budget)(verdW), verdPx, minPx, true);
        const verdictRect = { x: verdX, y, w: verdW, h: rowH };
        return {
            row, verdict,
    ...
    const counts = rows
        ...
        .reduce((a, v) => { a[v.kind] += 1; return a; }, { better: 0, worse: 0, noise: 0, unknown: 0 });
    const parts = [
    ...
    const summaryLines = parts.length === 0
        ? ["no rows"]
        : allUnmeasured
    ...
    const summary = fitLines(summaryLines, (0, text_1.budget)(sumW), subPx, minPx);
    ...
    const footer = fitSegments(footerSegments, (0, text_1.budget)(contentW), footPx, minPx).fit;
    ...
    const rule = { x: ruleX - ruleW, y: blockTop - Math.round(0.35 * margin), w: ruleW, h: bandUsed + Math.round(0.7 * margin) };
    ...
    const ruleLabel = fitOne(ruleLabelText, (0, text_1.budget)(ruleLabelW), ruleLabelPx, minPx);
    const ruleLabelRect = { x: rule.x + rule.w - ruleLabelW, y: rule.y - ruleLabelH, w: ruleLabelW, h: ruleLabelH };
    ...
    const kindsPresent = ["better", "noise", "worse", "unknown"].filter((k) => out.some((r) => r.verdict.kind === k));
    const legendKeys = [
    ...
    const legendWidth = (px, keys) => {
        const w = Math.max(6, Math.round(px * 0.85));
        const gap = Math.max(1, Math.round(px * 0.22));
        return keys.reduce((acc, k) => {
            const lw = Math.ceil((0, text_1.widthOf)(k.text, px) / 0.94) + 4;
            return acc + k.kinds.length * w + (k.kinds.length - 1) * gap + Math.round(0.4 * px) + lw + Math.round(1.2 * px);
        }, 0);
    };
    let legendShown = legendKeys;
    while (legendPx > minPx && legendWidth(legendPx, legendShown) > contentW)
        legendPx = Math.max(minPx, Math.round(legendPx * 0.92));
    if (legendWidth(legendPx, legendShown) > contentW && legendShown.length > 2)
        legendShown = legendShown.slice(0, 2);
    let lx = margin;
    const legend = legendShown.map(({ key, text, kinds }) => {
        ...
        const labelRect = { x: swEnd + Math.round(0.4 * legendPx), y: legendY, w: Math.ceil(label.width / 0.94) + 4, h: legendH };
 ...
 */
function benchLayoutContract(L) {
    const text = [
        (0, layout_1.textFitsMeasured)("title", L.title.text, L.title.px, L.title.width),
        (0, layout_1.textFitsMeasured)("summary", L.summary.text, L.summary.px, L.summary.width),
        (0, layout_1.textFitsMeasured)("rule-label", L.ruleLabel.text, L.ruleLabel.px, L.ruleLabel.width),
        (0, layout_1.textFitsMeasured)("footer", L.footer.text, L.footer.px, L.footer.width),
        ...(L.subtitle ? [(0, layout_1.textFitsMeasured)("subtitle", L.subtitle.text, L.subtitle.px, L.subtitle.width)] : []),
        ...L.rows.flatMap((r, i) => [
            (0, layout_1.textFitsMeasured)(`name-${i}`, r.name.text, r.name.px, r.name.width),
            (0, layout_1.textFitsMeasured)(`values-${i}`, r.values.text, r.values.px, r.values.width),
            (0, layout_1.textFitsMeasured)(`verdict-${i}`, r.verdictFit.text, r.verdictFit.px, r.verdictFit.width),
        ]),
        ...L.legend.map((l) => (0, layout_1.textFitsMeasured)(`legend-${l.key}`, l.label.text, l.label.px, l.label.width)),
    ];
    return [
        ...text,
        ...
        ...(L.rows.some((r) => r.baseIv) ? [{ label: "iv-base", within: { xFrac: [0.24, 1] } }] : []),
        ...(L.rows.some((r) => r.candIv) ? [{ label: "iv-cand", within: { xFrac: [0.24, 1] } }] : []),
        ...["better", "worse", "noise", "unknown"]
            .filter((k) => L.rows.some((r) => r.verdict.kind === k))
            .map((k) => ({ label: `bar-${k}`, within: { xFrac: [0.24, 1] } })),
        ...
        ...["better", "worse", "noise"]
            .filter((k) => L.rows.some((r) => r.verdict.kind === k))
            .map((k) => ({ label: `pill-${k}`, within: { xFrac: [0.7, 1] } })),
        // Presence: the legend is how a stranger reads which bar is which.
        ...L.legend.map((l) => ({ label: `swatch-${l.key}`, within: { yFrac: [0.6, 1] } })),
    ];
}
/**
...
exports.BenchDeltaV1 = (0, template_utils_1.defineMosaicTemplate)({
...
exports.default = exports.BenchDeltaV1;
...
async function render(props, ctx) {
    ...
    if (raw.length === 0)
        throw new Error(`${ID}: rows must hold at least one { name, base, value }.`);
    const rows = raw.map((row, i) => {
        if (!row || typeof row !== "object")
            throw new Error(`${ID}: rows[${i}] is not an object.`);
        ...
        if (name.length === 0)
            throw new Error(`${ID}: rows[${i}].name must not be empty.`);
        ...
        if (base === null)
            throw new Error(`${ID}: rows[${i}].base must be a number greater than zero - every bar on the row is relative to it. Got ${JSON.stringify(r.base)}.`);
        const value = toNum(r.value);
        if (value === null || value < 0)
            throw new Error(`${ID}: rows[${i}].value must be a number of zero or more. Got ${JSON.stringify(r.value)}.`);
        const spread = { baseRange: null, range: null };
        for (const k of ["baseRange", "range"]) {
            const v = r[k];
            if (v === undefined || v === null || v === "")
                continue;
            const n = toNum(v);
            if (n === null || n < 0)
                throw new Error(`${ID}: rows[${i}].${k} must be a number of zero or more (it is a +/- spread). Got ${JSON.stringify(v)}.`);
            spread[k] = n;
        }
        return {
            name,
    ...
    const barColor = (k) => k === "better" ? accent : k === "worse" ? theme.worse : k === "noise" ? theme.noise : theme.unknown;
    ...
    const ivColor = (c) => mix(String(c), String(theme.ink), 0.55);
    const pieces = [];
    const piece = (rect, importance, source) => pieces.push({ rect: { ...rect, importance }, source });
    const fill = (rect, importance, color, label, radius = 0.35) => piece(rect, importance, (0, template_utils_1.tag)({ ...(0, template_utils_1.makeColorTile)(color, radius > 0 ? { effects: { rounding: { cornerStyle: "rounded", borderRadius: radius } } } : {}) }, label));
    const text = (rect, importance, fit, color, label, o = {}) => { var _a, _b; return piece(rect, importance, (0, template_utils_1.tag)((0, text_1.textCell)({ text: fit.text, fontSize: fit.px, color, hAlign: (_a = o.hAlign) !== null && _a !== void 0 ? _a : "left", bold: o.bold, vAlign: (_b = o.vAlign) !== null && _b !== void 0 ? _b : "middle", label }), label)); };
    ...
    L.rows.forEach((r, i) => {
        const pill = pills[i];
        if (pill)
            fill(pill, 2, barColor(r.verdict.kind), `pill-${r.verdict.kind}`, 0.5);
    });
    for (const l of L.legend) {
        for (const sw of l.swatches) {
            ...
            const swatch = (0, template_utils_1.tag)({ ...(0, template_utils_1.makeColorTile)(color, { effects: { rounding: { cornerStyle: "rounded", borderRadius: 0.35 } } }) }, `swatch-${l.key}`);
            // `accent` paints the "better" swatch, so that is the rect that shows it.
            piece(sw.rect, 2, l.key === "cand" && sw.kind === "better" ? (0, template_utils_1.bindProp)(swatch, "accent") : swatch);
        }
    }
    // 4. the intervals, which sit over their own bar and over nothing else
    for (const r of L.rows) {
        if (r.baseIv)
            fill(r.baseIv, 3, ivColor(theme.base), "iv-base", 0.6);
        if (r.candIv)
            fill(r.candIv, 3, ivColor(barColor(r.verdict.kind)), "iv-cand", 0.6);
    }
    ...
    piece(L.titleRect, 2, (0, template_utils_1.bindProp)((0, template_utils_1.tag)((0, text_1.textCell)({ text: L.title.text, fontSize: L.title.px, color: theme.ink, hAlign: "left", bold: true, vAlign: "middle", label: "title" }), "title"), "title"));
    if (L.subtitle)
        piece(L.subtitleRect, 2, (0, template_utils_1.bindProp)((0, template_utils_1.tag)((0, text_1.textCell)({ text: L.subtitle.text, fontSize: L.subtitle.px, color: theme.dim, hAlign: "left", vAlign: "middle", label: "subtitle" }), "subtitle"), "subtitle"));
    text(L.summaryRect, 2, L.summary, theme.dim, "summary", { hAlign: "right" });
    L.rows.forEach((r, i) => {
        piece(r.nameRect, 2, (0, template_utils_1.bindPropPath)((0, template_utils_1.tag)((0, text_1.textCell)({ text: r.name.text, fontSize: r.name.px, color: theme.ink, hAlign: "left", bold: true, vAlign: "middle", label: `name-${i}` }), `name-${i}`), "rows", [i, "name"], "string"));
        ...
        piece(r.valuesRect, 2, (0, template_utils_1.tag)((0, text_1.textCell)({ text: r.values.text, fontSize: r.values.px, color: theme.dim, hAlign: "left", vAlign: "middle", label: `values-${i}` }), `values-${i}`));
        const pill = pills[i];
        if (pill)
            text(pill, 5, r.verdictFit, onColor(barColor(r.verdict.kind)), `verdict-${i}`, { hAlign: "center", bold: true });
        else
            text(r.verdictRect, 2, r.verdictFit, theme.dim, `verdict-${i}`, { hAlign: "right", bold: true });
    });
    ...
    piece(L.ruleLabelRect, 2, (0, template_utils_1.tag)((0, text_1.textCell)({ text: L.ruleLabel.text, fontSize: L.ruleLabel.px, color: theme.dim, hAlign: "right", vAlign: "middle", label: "rule-label" }), "rule-label"));
    for (const l of L.legend) {
        const cell = (0, template_utils_1.tag)((0, text_1.textCell)({ text: l.label.text, fontSize: l.label.px, color: theme.dim, hAlign: "left", vAlign: "middle", label: `legend-${l.key}` }), `legend-${l.key}`);
        piece(l.labelRect, 2, l.key === "base" ? (0, template_utils_1.bindProp)(cell, "baselineLabel") : l.key === "cand" ? (0, template_utils_1.bindProp)(cell, "candidateLabel") : cell);
    }
    ...
    return (0, layout_1.withLayoutIntent)(doc, ctx, { templateId: ID, constraints: benchLayoutContract(L), debug: props.debugLayout === true });
}
