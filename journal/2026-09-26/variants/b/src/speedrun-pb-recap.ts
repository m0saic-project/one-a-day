...
const WHY: WhySpec = {
  ...
  solution: [
    ...
    "The clip opens on the finished recap, replays the run with a fast-forwarded timer as each split lands at its share of the run, and ends where it began, so it loops. The graph is the time saved or lost on each segment. All moving text is gated drawtext: 16 rows cost two overlays.",
