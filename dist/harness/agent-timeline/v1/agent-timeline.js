...
exports.AgentTimelineV1 = exports.SAMPLE_PHASES = void 0;
exports.fmtTokens = fmtTokens;
exports.fmtUsd = fmtUsd;
exports.fmtDuration = fmtDuration;
exports.tickStepMs = tickStepMs;
...
const ERROR = "#f85149";
const NO_SHIP = "#e3b341";
const DEFAULT_ACCENT = template_utils_1.BRAND_ORANGE;
...
function roundTile(color, radius, label) {
    return { ...(0, template_utils_1.makeColorTile)(color, { effects: { rounding: { cornerStyle: "rounded", borderRadius: radius } } }), editor: { owner: "template", label } };
}
exports.AgentTimelineV1 = (0, template_utils_1.defineMosaicTemplate)({
    id: (0, types_1.asTemplateId)(ID),
    label: "Harness · Agent Timeline",
    version: 1,
    description: "How a day was made: the agent's phases as a waterfall - one lane per phase on a shared time axis, tool calls, tokens and the tools it leaned on, with the run totals and where the numbers came from. A harness fixture the why-tutorial renders as its last page; never a top-level pick.",
...
exports.default = exports.AgentTimelineV1;
async function render(props, ctx) {
    ...
    const clean = (v, fallback) => (v === undefined ? fallback : String(v).replace(/[^\x20-\x7e]/g, "?").replace(/\s+/g, " ").trim());
    ...
    const totalCalls = phases.reduce((a, p) => { var _a; return a + ((_a = p.calls) !== null && _a !== void 0 ? _a : 0); }, 0);
    const totalTokens = phases.reduce((a, p) => { var _a; return a + ((_a = p.tokens) !== null && _a !== void 0 ? _a : 0); }, 0);
    const totalUsd = phases.reduce((a, p) => { var _a; return a + ((_a = p.costUsd) !== null && _a !== void 0 ? _a : 0); }, 0);
    ...
    const pieces = [];
    const constraints = [];
    const put = (rect, importance, src) => pieces.push({ rect: { ...rect, importance }, source: src });
    const putText = (rect, importance, opts, bind) => {
        const cell = (0, text_1.textCell)(opts);
        put(rect, importance, bind ? (0, template_utils_1.bindProp)(cell, bind) : cell);
        constraints.push((0, layout_1.textFitsMeasured)(opts.label, opts.text, opts.fontSize, (0, text_1.widthOf)(opts.text, opts.fontSize, opts.bold)));
    };
    // ── header band ──
    ...
    put({ x: margin, y: Math.round((headerH - glyphSide) / 2), w: glyphSide, h: glyphSide }, 2, { ...(0, template_utils_1.brandGlyphTile)(template_utils_1.HEADER_M_GLYPH, template_utils_1.BRAND_ORANGE), editor: { owner: "template", label: "mark" } });
    ...
    if (header.length > 0)
        putText({ x: headerTextX, y: 0, w: halfW, h: headerH }, 1, { text: header, fontSize: (0, text_1.fitLine)(header, (0, text_1.budget)(halfW), headerPx, minPx, true), color: INK, hAlign: "left", bold: true, label: "run" }, "header");
    if (headerRight.length > 0)
        putText({ x: W - margin - halfW, y: 0, w: halfW, h: headerH }, 1, { text: headerRight, fontSize: (0, text_1.fitLine)(headerRight, (0, text_1.budget)(halfW), headerPx, minPx), color: INK_DIM, hAlign: "right", label: "agent" }, "headerRight");
    // ── title + subtitle ──
    let y = headerH + margin;
    const contentW = W - 2 * margin;
    if (title.length > 0) {
        const px = (0, text_1.fitLine)(title, (0, text_1.budget)(contentW), Math.round(0.06 * S), minPx, true);
    ...
    {
        const px = (0, text_1.fitLine)(subtitle, (0, text_1.budget)(contentW), Math.round(0.03 * S), minPx);
    ...
    footerLines.forEach((line, i) => {
        putText({ x: margin, y: footerY + i * footerLineH, w: contentW, h: footerLineH }, 1, { text: (0, text_1.ellipsize)(line, footerPx, (0, text_1.budget)(contentW)), fontSize: footerPx, color: INK_DIM, hAlign: "left", label: i === 0 ? "source" : "cost-note" }, i === 0 ? undefined : "costNote");
    });
    ...
    for (let t = 0, i = 0; t <= totalMs && i < 12; t += step, i++) {
        ...
        if (w < 8)
            break;
        putText({ x, y: bodyTop, w, h: axisH }, 1, { text: label, fontSize: (0, text_1.fitLine)(label, (0, text_1.budget)(w), axisPx, minPx), color: INK_DIM, hAlign: "left", vAlign: "bottom", label: `tick-${i}` });
    }
    phases.forEach((p, i) => {
        ...
        putText({ x: margin, y: stackY, w: labelW, h: nameH }, 1, { text: (0, text_1.ellipsize)(name, namePx, (0, text_1.budget)(labelW), true), fontSize: namePx, color: INK, hAlign: "left", bold: true, label: `phase-${i}` });
        if (meta)
            putText({ x: margin, y: stackY + nameH, w: labelW, h: metaH }, 1, { text: (0, text_1.ellipsize)(meta, metaPx, (0, text_1.budget)(labelW)), fontSize: metaPx, color: INK_DIM, hAlign: "left", label: `phase-meta-${i}` });
        ...
        if (showTokens && p.tokens !== undefined && p.tokens !== null && p.tokens > 0) {
            ...
            const need = Math.ceil((0, text_1.widthOf)(label, tokenPx)) + 2 * Math.round(0.25 * tokenPx);
            const after = trackX + trackW - (barX + barPx);
            if (after >= need)
                putText({ x: barX + barPx + Math.round(0.25 * tokenPx), y: laneY, w: need, h: laneH }, 2, { text: label, fontSize: tokenPx, color: INK_DIM, hAlign: "left", label: `tokens-${i}` });
        }
    });
    constraints.push({ label: "lane-wash", minWidthFrac: 0.4, within: { xFrac: [0.2, 1] } });
    for (const st of ["ok", "error", "no-ship"])
        if (phases.some((p) => { var _a; return ((_a = p.status) !== null && _a !== void 0 ? _a : "ok") === st; }))
            constraints.push({ label: `bar-${st}`, within: { xFrac: [0.2, 1] } });
